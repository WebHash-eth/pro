...
import { DeploymentLogger, generateDeploymentId } from "@/app/lib/deployment-logger";
 ...
 */
async function createTarGzArchive(sourceDir: string, outputPath: string): Promise<string> {
...
async function uploadToIPFSWithTimeout(
  ...
  return new Promise(async (resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      reject(new Error(`IPFS upload timed out after ${timeoutMs / 1000} seconds`));
    }, timeoutMs);
    ...
    try {
      ...
      const archivePath = path.join(os.tmpdir(), `webhash-${Date.now()}.tar.gz`);
...
async function prepareReactAppForIPFS(outputPath: string, projectType: string): Promise<void> {
    ...
    for (const file of files) {
      ...
      if (fs.statSync(filePath).isDirectory()) {
        ...
        if (fs.existsSync(subIndexPath)) {
          console.log(`Found index.html in ${file} directory. Moving to root...`);
          // Read the index.html content
          const indexContent = fs.readFileSync(subIndexPath, 'utf8');
          
          // Fix asset paths to include the subdirectory
          const fixedContent = indexContent.replace(/(src|href)="(?!http|\/\/|#)/g, `$1="${file}/`);
          
          // Write to root
          fs.writeFileSync(indexPath, fixedContent);
          console.log(`Created root index.html with fixed paths to ${file}/ directory`);
          indexFound = true;
          break;
        }
      }
...
<body>
  <div class="container">
    ...
    <a class="button" href="${appDir ? appDir + '/' : ''}index.html">Open React App</a>
  </div>
</body>
</html>
      `;
      fs.writeFileSync(indexPath, html);
      console.log(`Created root index.html with auto-redirect to ${appDir || 'root'} directory`);
    }
  } else {
  ...
  fs.writeFileSync(dnslinkPath, 'This site is best viewed through a proper IPFS gateway with DNSLink support.');
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const userId = session.userId || session.user?.email || session.user?.id || 'unknown';
    
    const { repoFullName, branch, buildCommand, outputDirectory, projectType } = await request.json();
    ...
    (async () => {
      try {
        ...
        try {
          ...
          // Clone the repository
          try {
            await logger.info(`Cloning repository ${repoFullName} branch ${branch}...`);
            await execAsync(`git clone --branch ${branch} ${cloneUrl} ${tempDir}`);
            await logger.success("Repository cloned successfully");
          } catch (error) {
            await logger.error(`Error cloning repository: ${error instanceof Error ? error.message : String(error)}`);
            return;
          }
        } catch (error) {
          await logger.error(`Error accessing repository: ${error instanceof Error ? error.message : String(error)}`);
          return;
        }
          ...
          if (fs.existsSync(path.join(tempDir, 'package.json'))) {
            try {
              ...
              if (packageJson.dependencies && packageJson.dependencies.next) {
                await logger.info("Next.js project detected. Only static exports are supported on IPFS.");
                await logger.info("Server components, API routes, and server-side rendering will not work.");
              ...
              // Generic Node.js project
                ...
                await logger.info("Node.js project detected. Only static content will work on IPFS.");
                detectedProjectType = 'static';
              }
            } catch (error) {
              await logger.error(`Error parsing package.json: ${error instanceof Error ? error.message : String(error)}`);
              detectedProjectType = 'static';
            }
          } else {
        ...
        if (fs.existsSync(path.join(tempDir, 'app')) || fs.existsSync(path.join(tempDir, 'pages'))) {
          const hasApiDir = fs.existsSync(path.join(tempDir, 'app/api')) || fs.existsSync(path.join(tempDir, 'pages/api'));
          
          if (hasApiDir) {
            await logger.info("WARNING: API routes detected. These will not function on IPFS as it's static hosting only.");
          }
          
          if (fs.existsSync(path.join(tempDir, 'next.config.js'))) {
            await logger.info("Next.js project detected. Some features may not work on IPFS.");
          }
        }
        ...
        if (detectedProjectType !== 'static') {
          try {
            ...
            const hasNpmLock = fs.existsSync(path.join(tempDir, 'package-lock.json'));
            ...
            await logger.error(`Error installing dependencies: ${error instanceof Error ? error.message : String(error)}`);
            return;
          }
        }
        ...
        try {
          ...
          const envResponse = await fetch(`${process.env.NEXTAUTH_URL}/api/env-variables/decrypt?repository=${repoFullName}`, {
            headers: {
          ...
          if (envResponse.ok) {
            ...
            if (Object.keys(envVariables).length > 0) {
              await logger.info(`Found ${Object.keys(envVariables).length} environment variables for this repository`);
              ...
              await logger.success("Environment variables injected successfully");
              ...
              if (detectedProjectType === 'react') {
                ...
                for (const [key, value] of Object.entries(envVariables)) {
                  ...
                  const prodKey = key.startsWith('REACT_APP_') ? key : `REACT_APP_${key}`;
              ...
              await logger.info("No environment variables found for this repository");
            }
          } else {
            await logger.info("Failed to fetch environment variables, continuing without them");
          }
        } catch (error) {
          await logger.error(`Error fetching environment variables: ${error instanceof Error ? error.message : String(error)}`);
          await logger.info("Continuing deployment without environment variables");
        }
          ...
          try {
            ...
            if (packageJson.scripts) {
              if (packageJson.scripts.build) {
                ...
                await logger.info(`Found build script: ${effectiveBuildCommand}`);
              } else if (packageJson.scripts.export && detectedProjectType === 'nextjs') {
                effectiveBuildCommand = 'npm run export';
                await logger.info(`Found export script: ${effectiveBuildCommand}`);
              } else {
                await logger.info("No build script found in package.json");
              }
            }
          } catch (error) {
            await logger.error(`Error determining build command: ${error instanceof Error ? error.message : String(error)}`);
          }
        } else if (effectiveBuildCommand) {
          await logger.info(`Using provided build command: ${effectiveBuildCommand}`);
        } else {
        ...
        if (effectiveBuildCommand && detectedProjectType !== 'static') {
          try {
            await logger.info(`Running build command: ${effectiveBuildCommand}`);
            
            // Check for Next.js specific errors before building
            const isNextJs = fs.existsSync(path.join(tempDir, 'next.config.js'));
            if (isNextJs) {
              // Add a warning about Next.js limitations
              await logger.info("WARNING: Next.js detected. Only static exports will work on IPFS.");
              await logger.info("Server components, API routes, and server-side rendering will not function.");
              
              // Check for common Next.js issues
              const hasApiRoutes = fs.existsSync(path.join(tempDir, 'app/api')) || fs.existsSync(path.join(tempDir, 'pages/api'));
              if (hasApiRoutes) {
                await logger.info("WARNING: API routes detected in Next.js app. These will not function on IPFS.");
              }
              ...
              if (fs.existsSync(nextConfigPath)) {
                try {
                  await logger.info("Modifying next.config.js to enable static export...");
                  ...
                  if (!nextConfigContent.includes("output: 'export'") && !nextConfigContent.includes('output: "export"')) {
                    ...
                    if (nextConfigContent.includes('module.exports')) {
                      ...
                      await logger.success("Modified next.config.js to enable static export");
                    } else {
                      ...
                      await logger.success("Created new next.config.js with static export enabled");
                    }
                  } else {
                    await logger.info("Static export already enabled in next.config.js");
                  }
                } catch (error) {
                  await logger.error(`Error modifying next.config.js: ${error instanceof Error ? error.message : String(error)}`);
            ...
            await logger.error(`Error running build command: ${error instanceof Error ? error.message : String(error)}`);
            
            // Check for Next.js specific errors
            const errorMessage = error instanceof Error ? error.message : String(error);
            if (errorMessage.includes('<Html>') || errorMessage.includes('next/document')) {
              await logger.error("Build failed: This appears to be a Next.js app with server components.");
              await logger.error("Next.js apps with server components, API routes, or server-side rendering cannot be deployed to IPFS as-is.");
              await logger.error("IPFS only supports static content. Consider using a static site generator or a client-side only React app.");
              return;
            }
        ...
        if (!effectiveOutputDirectory) {
          ...
          logger.info(`Using specified output directory: ${effectiveOutputDirectory}`);
          
          // Check if the specified output directory exists
          if (!fs.existsSync(path.join(tempDir, effectiveOutputDirectory))) {
            logger.info(`Specified output directory '${effectiveOutputDirectory}' not found. Attempting to auto-detect...`);
        ...
        if (!fs.existsSync(outputPath)) {
          logger.error(`Output directory '${effectiveOutputDirectory}' not found. Please check your build configuration.`);
          return;
        }
        
        // For Next.js static exports, ensure we're using the 'out' directory
        if (detectedProjectType === 'nextjs' && effectiveOutputDirectory === '.next') {
          ...
          try {
            ...
            await logger.error(`Error exporting Next.js app: ${error instanceof Error ? error.message : String(error)}`);
        ...
        if (fs.existsSync(nodeModulesPath)) {
          await logger.info("Removing node_modules from output directory to reduce size...");
        ...
        // Upload to IPFS using Lighthouse with timeout
        try {
          await logger.info(`Uploading directory ${finalOutputPath} to IPFS...`);
          await logger.info(`This may take a while depending on the size of your project.`);
          
          // List files to be uploaded (for debugging)
          const files = fs.readdirSync(finalOutputPath);
          await logger.info(`Files to upload (${files.length} items): ${files.slice(0, 10).join(', ')}${files.length > 10 ? '...' : ''}`);
          
          // Check directory size
          const dirSizeMB = getDirectorySizeInMB(finalOutputPath);
          await logger.info(`Output directory size: ${dirSizeMB.toFixed(2)} MB`);
          
          // Warn if directory is large
          if (dirSizeMB > 100) {
            await logger.info(`WARNING: Large directory size (${dirSizeMB.toFixed(2)} MB) may cause upload timeouts`);
          }
          
          // Upload with timeout
          await logger.info("Starting IPFS upload...");
          const response = await uploadToIPFSWithTimeout(finalOutputPath, apiKey);
          ...
          await logger.success(`Deployment completed successfully with CID: ${cid} and saved to database with ID: ${deployment._id}`);
          ...
          await logger.error(`Error uploading to IPFS: ${error instanceof Error ? error.message : String(error)}`);
          ...
          try {
            ...
            await deployment.save();
            await logger.info(`Failed deployment saved to database with ID: ${deployment._id}`);
            
            // Wait for all logs to be stored
            await logger.waitForPendingLogs();
          } catch (dbError) {
            await logger.error(`Error saving failed deployment to database: ${dbError instanceof Error ? dbError.message : String(dbError)}`);
          }
          
          // Check if it's a timeout error and provide more helpful guidance
          const errorMessage = error instanceof Error ? error.message : String(error);
          if (errorMessage.includes("timed out")) {
            await logger.error("Your project may be too large for direct upload. Try reducing the build size by optimizing assets or removing unnecessary files.");
          }
        }
      } finally {
        ...
        try {
          ...
          await logger.error(`Error cleaning up temporary directory: ${cleanupError instanceof Error ? cleanupError.message : String(cleanupError)}`);
        }
      }
    })().catch(async error => {
      await logger.error(`Unhandled error in deployment process: ${error instanceof Error ? error.message : String(error)}`);
    ...
    await logger.error(`Error starting deployment: ${error instanceof Error ? error.message : String(error)}`);
    
    // Wait for all logs to be stored
    await logger.waitForPendingLogs();
    
    return NextResponse.json(
      { error: `Failed to deploy to IPFS: ${error instanceof Error ? error.message : String(error)}` },
      { status: 500 }
    );
  }
} 